...
Calls core.text_to_sql directly (no HTTP API layer).
Uses @st.fragment so sending a message only re-renders the chat section.
"""
import streamlit as st
import pandas as pd
from core.text_to_sql import chat_with_data
...
def render_chat_interface():
    ...
    if submitted and user_input.strip():
        ...
        # Call text-to-SQL pipeline directly
        with st.spinner("🤖 Thinking…"):
            try:
                resp = chat_with_data(user_input.strip(), db, table)
                st.session_state["chat_history"].append({
                    "role": "agent",

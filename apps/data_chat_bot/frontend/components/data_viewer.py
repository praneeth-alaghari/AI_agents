...
Displays table data with configurable page size and Next/Previous navigation.
Uses @st.fragment so pagination clicks only re-render this section, not the
entire page.
"""
import streamlit as st
import pandas as pd
from services.api_client import fetch_table_data
...
def render_data_viewer():
    ...
    # ── Fetch data (cached by api_client) ────────────────────────────────
    try:
        data = fetch_table_data(db, table, page=current_page, page_size=page_size)

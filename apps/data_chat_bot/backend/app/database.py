...
def list_tables(db_name: str) -> list[dict]:
    """Return tables in the *public* schema of the given database."""
...
def get_table_row_count(db_name: str, table_name: str) -> int:
    """Return the total number of rows in a table (uses exact count)."""
    engine = _engine(db_name)
    with engine.connect() as conn:
...
def get_table_data(
    ...
    Returns
    -------
    dict with keys: columns, rows, total_rows, page, page_size, total_pages
    """
    engine = _engine(db_name)
...
# ─── Date range helper (context for the LLM) ───────────────────────────────
